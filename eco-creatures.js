...
const SPECIES_DEFS = {
    ...
    jellyfish:  { diet:'herb', baseColor:'#dd88ff', size:[8,16],   speed:[0.4,1.0],  sense:80,  reproduce:0.0015, hunger:80, seekMate:120, mateMin: 80, mateR: 2.5*80, activeAtNight:true,  minBreedAge:300,  breedCooldown:400,  litterSize:[2,3], maxAge:[2500,5000], spawnEnergy:80, maxEnergy:200},
    manta:      { diet:'herb', baseColor:'#00fff5', size:[14,24],  speed:[0.3,0.8],  sense:90,  reproduce:0.0015, hunger:80, seekMate:120, mateMin: 80, mateR: 2.5*90, activeAtNight:true, minBreedAge:350,  breedCooldown:430,  litterSize:[2,3], maxAge:[3000,6000], spawnEnergy:80, maxEnergy:200},
    seahorse:   { diet:'herb', baseColor:'#ff6ec7', size:[6,12],   speed:[0.2,0.5],  sense:70,  reproduce:0.0012, hunger:70, seekMate:120, mateMin: 80, mateR: 2.5*70, activeAtNight:true, minBreedAge:250,  breedCooldown:350,  litterSize:[2,4], maxAge:[2000,4000], spawnEnergy:80, maxEnergy:200},
    shark:      { diet:'carn', baseColor:'#cc00ff', size:[18,32],  speed:[0.8,1.4],  sense:120, reproduce:0.0010, hunger:80, seekMate:120, mateMin: 80, mateR: 3.5*120,  activeAtNight:true,  minBreedAge:700,  breedCooldown:900,  litterSize:[1,3], maxAge:[4000,7000], spawnEnergy:90, maxEnergy:200},  // 200→300
    anglerfish: { diet:'carn', baseColor:'#ff2d78', size:[14,26],  speed:[0.5,1.0],  sense:130, reproduce:0.0010, hunger:80, seekMate:120, mateMin: 80, mateR: 3.5*120,  activeAtNight:true,  minBreedAge:600,  breedCooldown:850,  litterSize:[1,3], maxAge:[4000,7000], spawnEnergy:90, maxEnergy:200},  // 200→300
    leviathan:  { diet:'apex', baseColor:'#ff6b35', size:[40,80],  speed:[0.7,1.1], sense:160, reproduce:0.0008, hunger:100, seekMate:170, mateMin: 100, mateR: 3.5*200,  activeAtNight:true,  minBreedAge:1000, breedCooldown:1200, litterSize:[1,1], maxAge:[7000,9000], spawnEnergy:150, maxEnergy:250},  // 200→500
};
...
function updateCreature(c, planets, galaxies, stars, newChildren, suns) {
    ...
    if (c.age >= def.minBreedAge && c._breedCooldown <= 0
        ...
        const litter = Math.min(
            ...
            speciesCap - speciesCount   // ← was: POP_CAP - creatures.length - newChildren.length
        );
